    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #f8f9fa 0%, #f0f1f3 100%)" }}>
      ...
      <div className="p-6 lg:p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {summaryCards.map((s: typeof summaryCards[number]) => (
        ...
        {enriched.length === 0 ? (
          ...
          <div className="space-y-4">
            {enriched.map((owner: EnrichedOwner) => {
              const statCards = [
              ...
              return (
                <div key={owner.id} className="bg-white rounded-2xl border border-white/80 shadow-sm overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-[#f0f0f0] border-b border-[#f8f9fa]">
                    {statCards.map((s: typeof statCards[number]) => (
                  ...
                  {owner.ownedProperties.length > 0 && (
                    <div className="divide-y divide-[#f8f9fa]">
                      {owner.ownedProperties.slice(0, 3).map((prop: OwnedProperty) => {
                        const propPaid = prop.bookings
                          .filter((b: PropertyBooking) => b.paymentStatus === "paid")
                          .reduce((s: number, b: PropertyBooking) => s + Number(b.totalAmount), 0);
                        return (
